...
import ru.dz.shipMaster.data.history.ParameterHistory.HistoryPoint;
import ru.dz.shipMaster.ui.misc.AlarmRegion;

/**
 * Normal (left to right) graph.
 * @author dz
 *
 */
...
public class BasicGraph extends AbstractGraph {
	...
	*/
	...
	public void paintDashComponent(Graphics2D g2d) {
	    ...
	    ParameterHistory history = getHistory();
	    if(history == null)
	    	return;
		...
		for( Iterator<HistoryPoint> i = history.iterator(); i.hasNext(); )
	    {
	    	HistoryPoint point = i.next();

	    	if( lmt < 0 ) lmt = point.getTime(); // time of fist point
	    	
    		double newXd = calcXPos( point.getTime() );
    		int newX = (int)newXd;
    		int newHeight = height-calcHeight(point.getValue()); 
	    ...
	    synchronized(ticksSema) {
	    	...
	    	for(Tick t : ticks)
	    	{
	    		...
	    		if( lastX >= 0 )
	    		{
		    		g2d.setColor(vis.indicatorColor);
	    			g2d.drawLine(lastX, lastHeight, newX, newHeight);
	...
	*/
	...
	{
		return (timeMillis-produceLeftMarginTime())*displaySpeed/1000;
	}
	/**
